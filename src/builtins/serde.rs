...
pub fn cog_serialize(mut state: CognitionState, w: Option<&Value>) -> CognitionState {
  ...
  let mut v1 = state.current().stack.pop().unwrap();
  ...
  state.pool.add_val(v1.value_stack().pop().unwrap());
  v1.value_stack().push(Value::Word(vword));
  state.current().stack.push(v1);
  state
}
...
pub fn add_builtins(state: &mut CognitionState) {
  ...
  // add_builtin!(state, "serde-save", cog_save);
  // add_builtin!(state, "serde-load", cog_load);
  // add_builtin!(state, "describe-fllibs", cog_describe_fllibs);
  // add_builtin!(state, "serialize-map", cog_serialize_map);
  // add_builtin!(state, "load-fllibs", cog_serde_load_fllibs);
}
